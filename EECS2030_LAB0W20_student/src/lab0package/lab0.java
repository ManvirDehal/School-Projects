...
public class lab0 {
	...
	public static double gaussian(double x, int sigma) {
		double first = Math.exp(-x*x / 2 * sigma * sigma);
		double second = Math.sqrt(2* Math.PI)*sigma;
		return first/second;
	}
	...
	*/
	public static int[] reverseOf(int[] inputarray) {
		...
		if (arrLen > 1)
		{
		for (int i = (arrLen-1), x = 0; 0 <= i; i-- , x++)
		{
			revArr[x] = inputarray[i];
		}
		return revArr;
		}
		else 
    ...
    public static boolean hasValidLengthAndSeparator(String s) {
    	...
    	if(s.length() == 5 && s.contains(":") && parts[0].length() == 2)
    	{
    		return true;
    	}
    	else return false;
    ...
    if (t.size() > 0)
    	{
    		   for (int x = 0; x < t.size(); x++)
    	       {
    		   	double temp = t.get(0);
    			t.remove(0);
    			double tempRads = Math.toRadians(temp);
    			t.add(tempRads);
       			}   
    	}
    }
	...
	public static List<Character> shuffle(List<Character> t) {
		...
		{
			firstHalf.add(t.get(x));
		}
		for (int y = t.size() / 2; y < t.size(); y++)
		{
			secondHalf.add(t.get(y));
		}
		for(int x = 0; x < 3; x++)
		{			
			end.add(firstHalf.get(x));
			end.add(secondHalf.get(x));
		}
		return end;
	}
}
